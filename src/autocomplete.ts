...
export class Autocomplete<T extends AutocompleteSuggestion> {
  ...
  constructor(options: AutocompleteOptions<T>) {
    ...
    this.options = {
      ...
      renderSuggestion: (suggestion, element) => {
        element.textContent = String(suggestion[this.options.displayProperty]);
      },
      ...options,
    };
  ...
  private selectSuggestion(suggestion: T): void {
    // Only update the input with the display property
    this.options.inputElement.value = String(
      suggestion[this.options.displayProperty]
    );
